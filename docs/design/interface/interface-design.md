...
- 「両方」を選択した場合は、タスク一覧（UI-008）とガントチャート（UI-009）を同一ページ内で横並び（左: タスク一覧、右: ガントチャート）に配置し、1 行 = 1 タスクを共有するレイアウトで 1 つの PDF に出力する。タスク行とガントバーの対応が一目で分かることを優先し、時間軸はページ幅に収まるよう自動スケールする（1 ページに収まらない行数の場合は行単位で改ページ）。
